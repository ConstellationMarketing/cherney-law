/**
 * Homepage2FieldEditor
 * Specialized CMS editor for /homepage-2 only.
 * Differences from HomePageFieldEditor:
 *  - Hero: no H1 Title, no Syndications Label; adds Hero Background Image + Hero Attorney Image
 *  - About: adds Syndications Label field (shown above attorney image on the live page)
 */
...
export default function Homepage2FieldEditor({ content, onChange }: Props) {
  ...
  return (
    <Accordion type="multiple" defaultValue={["hero"]} className="space-y-2">
      ...
      {/* ── ABOUT (Homepage-2 variant — includes Syndications Label) ── */}
      <AccordionItem value="about" className="border rounded-lg px-4">
        ...
        <AccordionContent className="space-y-4 pb-4">
          ...
          {/* Syndications Label — Homepage-2 only */}
          <div className="p-3 border border-green-200 rounded-lg bg-green-50 space-y-2">
            <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Syndications Label (Homepage-2 only)</p>
            <p className="text-xs text-green-600">This text appears above the attorney image in the About section, centered.</p>
            <Field label="Syndications Label Text">
              <Input
                value={(about as any).syndicationsLabel || ""}
                onChange={e => setAbout("syndicationsLabel", e.target.value)}
                placeholder="Featured in the following syndications:"
              />
            </Field>
          </div>
